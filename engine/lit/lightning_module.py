...
class TaskTrainer(LightningModule):
    def __init__(
        ...
        log_prefix=None,
        run_metrics=None,
        display_training_samples=False,
        display_validation_samples=True,
        display_sample_length=5,
        display_computational_graph=False,
        devices="auto",
        accelerator="auto",
    ):
        super().__init__()
    ...
    def log_hparams(self):
        ...
        for task_attr_name in dir(self.task):
            ...
            if isinstance(attr_value, torch.nn.Module):
                transformer_name = getattr(
                    attr_value, "name_or_path", attr_value.__class__.__name__
                )
        ...
        self.task_logger.log_hyperparams(
            params=hparams_dict, metrics=metrics_dict)
    ...
    def display_computational_graph(self, loss_tensor):
        ...
        graph = make_dot(loss_tensor, params=dict(
            self.task.named_parameters()))
        graph.format = "pdf"
    ...
    def on_fit_start(self):
        if self.task is None:
            raise AttributeError(
                "No task was provided to the TaskTrainer object.")
        self.log_hparams()
    ...
    def log_metrics(self, step_name, loss_info, batch_size):
        ...
        for field in loss_info.__dataclass_fields__:
            ...
            if field == "optimization":
                ...
                self.task_logger.log_distribution(
                    f"{step_name}/{field}", value, step=self.global_step
                )
    ...
    def _log_stored_histograms(self, step_name, display_top_k=15):
        ...
        for key, histogram_dict in self._stored_histogram_dict.items():
            ...
            self.task_logger.log_figure(
                f"{step_name}/{key}", fig, step=self.global_step
            )
    ...
    def training_step(self, input, batch_idx):
        ...
        if self.display_training_samples:
            samples_to_display = self.task.display_sample(
                train_loss_info, self.display_sample_length
            )
            self.log_text("train/samples", samples_to_display)

            histogram_data_to_display = self.task.display_histograms(
                train_loss_info)
    ...
    def validation_step(self, input, batch_idx):
        ...
        if self.display_validation_samples:
            if batch_idx == 0:
                samples_to_display = self.task.display_sample(
                    val_loss_info, self.display_sample_length
                )
    ...
    def fit(self, datamodule, *args, **kwargs):
        ...
        trainer = Trainer(
            ...
            logger=self.task_logger.pl_logger,
            max_steps=self.total_nb_steps,
            *args,
            **kwargs,
        )
