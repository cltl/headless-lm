...
_GLUE_TASK_CONFIG = {
    ...
    "rte": {
        "task_class": SentencePairClassification,
        "task_config": {"num_class": 2, "warmup_steps": 160},
        "nb_epochs": 20,
    },
    "sst2": {
        "task_class": SentenceClassification,
        "task_config": {"num_class": 2, "warmup_steps": 4200},
        "nb_epochs": 20,
    },
    "cola": {
        "task_class": SentenceClassification,
        "task_config": {"num_class": 2, "warmup_steps": 530},
        "metric": "mcc",
        "nb_epochs": 20,
    },
    "stsb": {
        "task_class": TextualSimilarity,
        "task_config": {"warmup_steps": 360},
        "metric": "spearman",
        "nb_epochs": 20,
    },
    "mrpc": {
        "task_class": SentencePairClassification,
        "task_config": {"num_class": 2, "warmup_steps": 230},
        "nb_epochs": 20,
    },
    "qqp": {
        "task_class": SentencePairClassification,
...
class GlueBenchmark:
    ...
    def __call__(self):
        for i, (task_name, task_attr) in enumerate(_GLUE_TASK_CONFIG.items()):
            ...
            task_datamodule = DataModule.from_datasets(
                "glue",
                task_name,
            ...
            trainer = TaskTrainer(
                ...
                run_metrics=self.run_metrics if i == 0 else None,
                logger=self.logger,
            )

            task_metric = task_attr.get("metric", "accuracy")
            checkpoint_callback = ModelCheckpoint(
                monitor=f"{task_name}/val/{task_metric}", mode="max", save_top_k=0
            )
            trainer.fit(
                task_datamodule,
